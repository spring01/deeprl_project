...
import numpy as np
import random
from memory_episode import Episode
...
class DRQNAgent(object):
    ...
    def fit(self, env):
        ...
        while len(self.memory) < self.args.num_burn_in:
            ...
            for ep_len in xrange(self.args.max_episode_length):
                ...
                if _every(ep_len, self.args.action_change_interval):
                    act = self.policy['random'].select_action()
                state, state_mem, reward, done = self.get_state(env, act)

                # store transition into episode
                episode.append(state_mem, act, reward, done)

            # store the entire episode into replay memory
        ...
        while iter_num <= self.args.num_train:
            ...
            episode = Episode(state_mem)
            history = History(state, self.args.timesteps)
            q_online = self.predict_online(history.get_seq_state())
    ...
    def predict_online(self, seq_state):
        return self.q_network['online'].predict([seq_state, self.null_act])[1]
    ...
    def get_batch(self):
        mini_batch = self.memory.sample_batch(self.args.timesteps, self.args.batch_size)
        input_b = []
        act_b = []
    ...
    def get_target(self, mini_batch, input_b_n, act_b):
        ...
        for qtn, (_, _, rew, _, db) in zip(q_target_b_n, mini_batch):
            full_reward = self.preproc.clip_reward(rew)
            if not db:
