...
from dqn.drqn2 import DRQNAgent
...
def create_model(window, input_shape, num_actions, model_name='dqn'):
    width = input_shape[0]
    model_input_shape = window, width, width, 1
    state = Input(shape=model_input_shape)
    if 'cheap' in model_name:
        conv1 = TimeDistributed(Conv2D(16, (8, 8), strides=(4, 4),
            padding='same', activation='relu'))(state)
        conv2 = TimeDistributed(Conv2D(32, (4, 4), strides=(2, 2),
            padding='same', activation='relu'))(conv1)
        feature = TimeDistributed(Flatten())(conv2)
